...
到這裡，再看一次圖七(b)，可以發現，決定出$h(21)=4$的過程當中，以二進位表示的**Key**$=21$的「每一個部分的bit」都用上了，二進位的$4$是由以二進位表示的$Key=21$之「頭：$101$」加上「中：$101$」再加上「尾：$010$」得到的，因為參與的「部位」變多了，那麼隨機性也就增加了，如此$h(Key)$便能得到較為隨機的結果。  
