...
如此便能把第$4$張紙牌加入(**insert**)「前$3$張排序過」的紙牌組合，得到$4$張排序過的紙牌組合。
...
* 定義變數`int j`來表示「已經排好序」的數列的「最後一個」元素之index。
    * 在此`j=0`，`array[j]=5`。
    * `int j`會不斷遞減，`j--`，來檢查`array[j]`是否比「目前欲處理」的資料還大，如果是，就將`array[j]`「往後移」。
...
以及`main()`，建立如圖二的矩陣，並進行`InsertionSort()`，並將矩陣元素以`PrintArray()`印出。
